...
- `GET /api/proxy/traces` returns persisted per-request `ApiProxyRequestTrace` records (model → route → target → scheduler actions → usage → outcome), newest first, plus `total` — the count of rows matching the filter regardless of pagination. Filters: `from=`/`to=` (inclusive ISO time range), `modelId=`, `sourceId=`, `targetId=`, `protocol=`, `endpoint=`, `ok=true|false`, `status=`, `errorCode=`, `cache=hit|store|coalesced|none`, `resumed=`, `stream=`, `translated=`, `minDurationMs=`. Pagination: `before` is an `at`-cursor, `limit` is clamped to 500.
- `GET /api/proxy/traces/facets` returns distinct filter values with occurrence counts over the whole history (models, sources, targets, endpoints, protocols, statuses, error codes; sources/targets carry a display `name` extracted from the stored trace JSON) — used to populate the Request-history page (`#/proxy/traces`) filter dropdowns.
...
Every request emits an `ApiProxyRequestTrace` recorded by the Observer `proxy/stats.ts`: in-memory hourly counters keyed off `trace.at` (reseeded at boot from history) plus a persistent trace history in SQLite `proxy_request_traces` (`proxy/traces-repository.ts`) — one row per trace with indexed filter columns (`at`, `model_id`, `source_id`, `target_id`, `ok`, …) and the full validated trace as JSON, re-parsed with `safeParse` on read so schema evolution self-heals by dropping incompatible rows. Retention is 30 days, enforced at boot and hourly on insert; the prune pass also deletes `data/proxy-requests/` capture-artifact directories older than the same cutoff (by the timestamp embedded in the directory name, so orphaned artifacts are swept too). Tokens and rate are metered on **both** the resumable path and the plain forwarder, via `proxy/usage-meter.ts`:
