...
Every facade request resolves its `Authorization: Bearer`/`x-api-key` against the request-source registry (`proxy/sources.ts:resolveApiProxyRequestSource`) into one of four outcomes: `source` (active source — stamps `trace.sourceId`/`sourceName`), `disabled` (key belongs to a disabled source — still stamped, so blocked attempts stay attributable in traces), `unknown` (key matches nothing) and `anonymous` (no key). The gate (`apiProxyRequestSourceRejection`) applies in `proxy/protocol-endpoint.ts` before model resolution (auth errors precede `not_found`, so anonymous callers cannot probe model existence, and rejected requests never touch the route chain or response cache) and on `GET /v1/models`:

- A `disabled` source is **always** rejected with `403` and the source's admin-set `blockedMessage` (default: `Source <name> is disabled by the administrator.`), regardless of the anonymous policy.
...
Rejections are shaped per facade by the adapter `authError` hook: OpenAI-shaped `{error:{type,code,message}}` and Anthropic-shaped `{type:"error",error:{type,message}}`, with `type` `authentication_error` (401) / `permission_error` (403) — both SDK families map the exception class off the HTTP status. Codes: `arriero_proxy_source_required` (missing key), `invalid_api_key` (unknown key), `arriero_proxy_source_disabled` (blocked source). Rejected requests are still recorded as traces (status 401/403) and count in stats. Node-to-node delegation (`/api/proxy/serve`) bypasses this gate — the source policy applies on the entry node that faces the client.
