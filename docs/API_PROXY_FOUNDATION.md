...
- **Deferred seams** (documented, unchanged): cross-node delegation still injects `return_progress` before the sending node knows the remote engine (harmless; the delegate payload should carry capability flags once a second engine exists); `stateFromHealth` in `proxy/runtime.ts` is the single spot where per-engine health→state mapping would plug in; `parseInstanceParallelLimit` in `domain-admission.ts` reads llama `--parallel`/`-np` and belongs in the descriptor when another engine needs a concurrency cap. The force-answer reasoning tail lives in `proxy/force-answer.ts` (`<think>` delimiters); reasoning-tag conventions are a property of the model, not the engine, so its eventual home is a per-model option.
