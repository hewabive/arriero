...
- `GET /api/proxy/traces?limit=&before=&modelId=&sourceId=&targetId=&ok=` returns persisted per-request `ApiProxyRequestTrace` records (model → route → target → scheduler actions → usage → outcome), newest first; `before` is an `at`-cursor for pagination, `ok=true|false` filters by outcome, limit is clamped to 500.
