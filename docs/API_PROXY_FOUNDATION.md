...
- Files upload, listing, metadata, deletion, and content download are available under both `/v1/files` and `/proxy/v1/files`. These model-independent operations select an external OpenAI endpoint via `X-Arriero-Endpoint`, the configured `filesEndpointId` default, or automatically when only one is eligible; see [Files API](EXTERNAL_PROVIDERS.md#files-api).
...
Every facade request resolves its `Authorization: Bearer`/`x-api-key` against the request-source registry (`proxy/sources.ts:resolveApiProxyRequestSource`) into `source` (key matched — carries the source's `enabled`/`blockedMessage` and stamps `trace.sourceId`/`sourceName` whether or not it is enabled, so blocked attempts stay attributable in traces), `unknown` (key matches nothing) or `anonymous` (no key). `apiProxyRequestGate(headers)` composes that resolution with the `allowAnonymous` policy into a rejection; it runs in `proxy/protocol-endpoint.ts` before the request body is read (so an unauthorized caller never gets its payload parsed, and auth errors precede `not_found` — anonymous callers cannot probe model existence, and rejected requests never touch the route chain or response cache) and on `GET /v1/models` and Files operations:

- A `disabled` source is **always** rejected with the source's admin-set `blockedMessage` (default: `Source <name> is disabled by the administrator.`), regardless of the anonymous policy.
- With `allowAnonymous:false` in `config/proxy/settings.json` (`proxy/settings.ts`, `GET`/`PATCH /api/proxy/settings`, toggle on `#/proxy/sources`), `anonymous` and `unknown` requests are rejected too; only configured active source keys pass. Their messages are the admin-set `anonymousBlockedMessage` / `unknownKeyBlockedMessage` settings (empty = built-in defaults), edited next to the toggle on `#/proxy/sources`. The default (`allowAnonymous:true`) keeps the original labeling-only behavior — unknown/missing keys pass through as anonymous.

Every gate rejection uses status `423` with error `type` `permission_error` — never `401`/`authentication_error`, which Claude Code misclassifies as expired authentication, opening its login flow and retrying. Rejected requests keep `trace.sourceId`/`sourceName` but no `modelId` — the body is never read. Rejections are shaped per facade by the adapter `authError` hook: OpenAI-shaped `{error:{type,code,message}}` and Anthropic-shaped `{type:"error",error:{type,message}}`. Codes: `arriero_proxy_source_required` (missing key), `invalid_api_key` (unknown key), `arriero_proxy_source_disabled` (blocked source). Rejected model-based requests are still recorded as traces (status 423) and count in stats. Node-to-node delegation (`/api/proxy/serve`) bypasses this gate — the source policy applies on the entry node that faces the client; the entry's resolved source travels in the serve payload's `origin` and is stamped into the owning node's trace and in-flight entry, so the peer's Request history attributes delegated calls (`docs/FEDERATION.md` § Remote-request telemetry depth).
