...
A control surfaced in the web UI (`Proxy targets` → Runtime → in-flight request) lets the operator force an answer from `thinking`, finish while keeping the generated prefix, or cancel the request. The registry exposes three independent capability records (`forceAnswer`, `finish`, `cancel`) with availability and a reason instead of deriving every button from one `interruptible` boolean. `POST /api/proxy/inflight/:id/control/:action` dispatches the registered handler; the older action-specific routes remain compatibility aliases.

The in-flight registry (`proxy/inflight.ts`) accumulates streamed reasoning, answer text and tool calls per request (capped tail buffers, full counts tracked) on every streaming path. `stream-inspector.ts` owns protocol parsing, usage, live callbacks, malformed-frame accounting and terminal classification for pass-through, translated and resumable traffic, so a bridge does not lose control metadata or tool generation.

For current llama-server generations the engine descriptor advertises native reasoning control. The proxy adds `reasoning_control: true`, captures the streamed completion id, and sends `{id, action:"reasoning_end", model}` to `/v1/chat/completions/control`. This keeps the same sampler and stream alive, and works on ordinary OpenAI and translated Anthropic requests without a compute-domain lease. The button is `not-ready` until the id arrives and `too-late` after the stream leaves `thinking`.

Older managed resumable paths retain the abort/resume fallback. `runResumableUpstreamAttempt` reports the aborted control as a distinct `interrupted` outcome (vs `preempted`/`consumer-gone`). `runResumableForward` then re-issues **without** yielding the lease, building the resume tail from `buildForceAnswerTail(state.reasoningText)` = `<think>\n{reasoning}\n</think>\n\n` instead of the usual visible-text tail, and — unlike the automatic thinking case — does **not** reset `state.reasoningText`. llama.cpp's assistant-prefill continues from the closed think block, so the upstream streams the answer (`content`) into `state.text`; the final response carries both the preserved `reasoning_content` and the new answer. If the forced attempt is itself preempted, the resume tail stays `forceAnswerPrefix + state.text`, keeping the think-close ahead of the answer-so-far.

Finish and Cancel no longer share one abort signal. Cancel aborts the upstream and discards the response. Finish is offered only where the proxy can preserve a coherent result: buffered resumable requests synthesize from their neutral state, while live OpenAI Chat and Anthropic streams are cut at an SSE frame boundary and receive a protocol-correct terminal (including `signature_delta` before an open Anthropic thinking block closes). Unsupported response shapes report `not-supported` instead of displaying a button that will corrupt or truncate their protocol.

The `<think>`/`</think>` reconstruction applies only to the compatibility fallback and remains its one model-specific assumption: `reasoning_content` reaches the proxy with the model's think tags already stripped by llama.cpp, so the prefix is rewrapped with the de-facto default markers (Qwen3, DeepSeek-R1, most local reasoning models). Native reasoning control avoids that assumption.
