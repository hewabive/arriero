...
| Old `--model-vocoder`/`--hf-repo-v`/`--hf-file-v`/`--tts-use-guide-tokens` server rows | No current server artifact | These flags were removed from source and the b10276 binary, and the generated upstream README no longer lists them either. Current server audio input is an mmproj case, while speech generation belongs to the separate `llama-tts` executable. |
