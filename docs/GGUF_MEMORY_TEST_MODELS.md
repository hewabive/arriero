...
Reviewed: **2026-08-04** against Arriero `e851c55` and llama.cpp
...
| SWA upper-bound path and tiny multimodal projector | [`ggml-org/tinygemma3-GGUF` / `tinygemma3-Q8_0.gguf`](https://huggingface.co/ggml-org/tinygemma3-GGUF/blob/c287502cd9e278dac8eed805c112cce5d0081e0b/tinygemma3-Q8_0.gguf) plus [`mmproj-tinygemma3.gguf`](https://huggingface.co/ggml-org/tinygemma3-GGUF/blob/c287502cd9e278dac8eed805c112cce5d0081e0b/mmproj-tinygemma3.gguf) | 47.2 MB + 1.0 MB | **anchor**. Eight Gemma 3 blocks with a 4096-token sliding window but one KV width, so the estimator intentionally reports the SWA upper-bound warning. The projector covers default GPU placement and `--no-mmproj-offload` without a large download. SHA-256 `7566ae7219c93ea2ecc692a931ee122d30c55261d0e2c3347acb8b939d2e9abd` and `93c2ba8c34574dd8f2dfda64931fc20943de2f941bfe03e6e9eca68951b80604`. |
...
| MLA and large MoE | [`bartowski/DeepSeek-Coder-V2-Lite-Instruct-GGUF` / `DeepSeek-Coder-V2-Lite-Instruct-IQ2_XS.gguf`](https://huggingface.co/bartowski/DeepSeek-Coder-V2-Lite-Instruct-GGUF/blob/8f248fa2072348f77a8bc37754e470de1f61866e/DeepSeek-Coder-V2-Lite-Instruct-IQ2_XS.gguf) | 5,967.4 MB | **gap canary**. `deepseek2` with MLA and 78 expert tensors. Arriero currently reports low confidence and omits the entire MLA cache; at 4096 context, `llama-fit-params` projected 1080 MiB that the analytical estimate did not include. SHA-256 `13106ce47dee9380b03ff27e88c87076604a01c85fea9fab191f827c234b46d3`. |
...
## Classic, role-specific, and recurrent gap canaries
...
| --- | --- | ---: | --- |
| Classic GPT-2 MHA | [`mradermacher/tiny-random-gpt2-GGUF` / `tiny-random-gpt2.f16.gguf`](https://huggingface.co/mradermacher/tiny-random-gpt2-GGUF/blob/ce6c3b782e51caef90f6bb1e7cae292afea6f878/tiny-random-gpt2.f16.gguf) | 16.0 MB | **gap canary**. The current estimator looks for separate `attn_k`/`attn_v` tensors, while GPT-2 uses a fused projection. It therefore reports zero KV and low confidence; `llama-fit-params` reports nonzero context. SHA-256 `79fa9fac98ece8d99627f0b28302d2698782b05e0df2de3c0b360f6be79fe0f8`. |
| Embedding/encoder-only BERT | [`ggml-org/e5-small-v2-Q8_0-GGUF` / `e5-small-v2-q8_0.gguf`](https://huggingface.co/ggml-org/e5-small-v2-Q8_0-GGUF/blob/14aa7ca88ca627e2ea0051929636561158e289ab/e5-small-v2-q8_0.gguf) | 36.7 MB | **gap canary**. Run as an embedding instance. The reviewed estimator incorrectly treated BERT K/V projection weights as a persistent decoder KV cache and returned high confidence, while `llama-fit-params` context was zero. This is a particularly important false-confidence regression test. SHA-256 `afdfb5c342d2efc2a051c426dd1d00913495d5f2bbceaea100d2f3892aa31cbc`. |
| Reranker/classifier role | [`ggml-org/models` / `jina-reranker-v1-tiny-en/ggml-model-f16.gguf`](https://huggingface.co/ggml-org/models/blob/499bc8821c6b12b4e53c5bffcb21ec206f212d81/jina-reranker-v1-tiny-en/ggml-model-f16.gguf) | 67.5 MB | **gap canary**. Upstream llama.cpp server fixture with a classifier head. Use with `--reranking`. Like E5, the reviewed estimator incorrectly allocated decoder KV and overestimated compute by about 10x while returning high confidence. SHA-256 `ad9f450c1053a431e2e3746d1f9f7768fb9183cf0796e046983a3449dca093c2`. |
| Pure Mamba SSM | [`QuantFactory/mamba-130m-hf-GGUF` / `mamba-130m-hf.Q2_K.gguf`](https://huggingface.co/QuantFactory/mamba-130m-hf-GGUF/blob/b459c7cf440d850438589a45e8e3b55f33211c42/mamba-130m-hf.Q2_K.gguf) | 69.3 MB | **gap canary**. Its metadata lacks `ssm.group_count`, so Arriero detects recurrent tensors but omits the state and returns low confidence. The state is context-independent in `llama-fit-params`. SHA-256 `58ab1786685f602ce4900d6c9a0a1f1abfaebb096f2ef082e60f2cdd9fc6dd42`. |
| Current RWKV-7 recurrent layout and IQ1_S | [`RemySkye/rwkv7-g1h-1.5b-i1-GGUF` / `rwkv7-g1h-1.5b-20260710-ctx10240.i1-IQ1_S.gguf`](https://huggingface.co/RemySkye/rwkv7-g1h-1.5b-i1-GGUF/blob/aece029dca2fda812f7a2a960067a36d2b26e3ed/rwkv7-g1h-1.5b-20260710-ctx10240.i1-IQ1_S.gguf) | 497.2 MB | **gap + format canary**. Arriero recognizes recurrent tensor names but has no RWKV state geometry, so it omits the 49 MiB projected by `llama-fit-params`. Also covers IQ1_S weight byte accounting. SHA-256 `1fd2a536e0c8e7d4a0890dc5a33b91352e5e11c7884ec353177a9f516d401e41`. |
...
| MXFP4 MoE | [`ggml-org/gpt-oss-20b-GGUF` / `gpt-oss-20b-MXFP4.gguf`](https://huggingface.co/ggml-org/gpt-oss-20b-GGUF/blob/ef9b12f2ff56c69cf32153a02784e7a3c88bf524/gpt-oss-20b-MXFP4.gguf) | 12,109.6 MB | **format anchor** for native MXFP4 MoE and the modern `gpt-oss` layout. Arriero reported no unknown tensor types and its weight sum matched `llama-fit-params` to 1 MiB. Its single-width SWA remains an upper-bound gap. SHA-256 `27cd6c432c7672cb812a92f611cf3ba7bbc35928262bb1e1253ff4ee6ae35901`. |
...
| CPU, partial, full GPU | `--n-gpu-layers 0`, a finite middle value, and `all`/`999` | `stories15M-q4_0` first, then one real model | Tensor weights move by layer; input embedding remains on host and compute follows the output layer. |
...
| Flash attention | `--flash-attn off` vs `on`, including quantized KV | TinyGemma or Qwen3.5 | Resolved flag changes; current analytical compute formula should otherwise stay stable. Compare to the matching binary. |
...
| TinyGemma 3 | Loaded with its projector in the CUDA server and passed `/health`; the estimator reported the intended single-width SWA upper-bound warning. |
| Qwen3.5 0.8B | Loaded; modeled recurrent + attention context matched across context sizes, KV types, and ubatch cases. |
| Granite 4.0 H Micro | Loaded; weights matched displayed MiB and modeled recurrent + attention context matched exactly. |
| Gemma 4 E2B | Loaded as the full model + projector + MTP trio in the CUDA server; logs confirmed MTP cache sharing and a real completion accepted draft tokens. Distinct-width SWA and shared-KV context matched within 0-2 MiB, and projector/MTP bytes were included by the analytical composite path. |
| DeepSeek V2 Lite | Loaded; low-confidence MLA warning reproduced the known complete KV omission. |
| tiny GPT-2 | Loaded; low-confidence zero-KV gap reproduced. |
| e5-small-v2 | Loaded; false high-confidence BERT KV allocation reproduced and must remain visible until fixed. |
| Jina tiny reranker | Loaded; the same false high-confidence encoder KV and roughly 10x analytical compute overestimate reproduced. |
| Mamba 130M and RWKV-7 | Loaded; missing recurrent-state warnings reproduced. |
| GPT-OSS 20B MXFP4 | Loaded on CPU and CUDA builds with no unknown tensor types; analytical weights matched `llama-fit-params` to 1 MiB. Its single-width SWA context remains an upper bound. |
...
Granite, Gemma, DeepSeek, and GPT-OSS rows. It did **not** close Arriero's GPU
placement warning: backend-specific model/compute allocations still differ from
the analytical per-pool attribution, so keep treating that branch as
source-derived until a dedicated per-device comparison is added.
