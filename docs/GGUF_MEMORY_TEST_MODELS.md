...
Reviewed: **2026-08-05** against the Arriero revision containing this document and llama.cpp
`6ea215d171fd31df943bf1ac8227129f2b963160` (`b10276`). Repository revisions,
...
| Coverage | Artifact | Size | Status and use |
| --- | --- | ---: | --- |
| Hybrid attention + recurrent state | [`unsloth/Qwen3.5-0.8B-GGUF` / `Qwen3.5-0.8B-Q4_K_M.gguf`](https://huggingface.co/unsloth/Qwen3.5-0.8B-GGUF/blob/6ab461498e2023f6e3c1baea90a8f0fe38ab64d0/Qwen3.5-0.8B-Q4_K_M.gguf) | 532.5 MB | **anchor**. `qwen35`, six attention and 18 recurrent blocks, with all four SSM metadata fields consumed by Arriero. Analytical context matched `llama-fit-params` at all harness configurations. Its tied output is the main one-GPU placement anchor: analytical 497 MiB GPU + 199 MiB host weights versus fit 497 + 198 MiB. With the DSpark sidecar it also covers target recurrent rollback snapshots: 38.53/77.06/154.12 MiB RS at speculative depth 1/3/7. SHA-256 `bd258782e35f7f458f8aced1adc053e6e92e89bc735ba3be89d38a06121dc517`. |
...
| Step 3.5, MiMo2, and Hy3 family-specific MTP contexts | [`AesSedai/Step-3.5-Flash-GGUF` at `636522b`](https://huggingface.co/AesSedai/Step-3.5-Flash-GGUF/tree/636522be1ffc1bd2b69f322361e6338f10426e00); [`unsloth/MiMo-V2-Flash-GGUF` at `04c761f`](https://huggingface.co/unsloth/MiMo-V2-Flash-GGUF/tree/04c761fbebcf4f3239b1cfdafec85533dcc0dea3); [`vcruz305/Hy3-GGUF` at `41a7e6c`](https://huggingface.co/vcruz305/Hy3-GGUF/tree/41a7e6cf752c2b6901c1b89b0cd8d4fece224d6f) | 64.43 GiB (`IQ2_S`); 72.36 GiB (`UD-TQ1_0`); 85.45 GiB (`IQ1_M`) | Step 3.5 uses three MTP heads; MiMo2 combines three heads with fused QKV and iSWA; Hy3 selects another family-specific MTP graph. The qualified Qwen sidecars cover one dense NextN head and Gemma covers target-shared KV, so they are not equivalent. Arriero recognizes `step35`, `mimo2`, and `hy_v3`, includes readable weights/KV, and returns `low` until complete second-context buffers are hardware-qualified. |
...
| --- | --- | --- | --- |
| Context growth | `--ctx-size 4096`, `16384`, and a non-256/divisible value with explicit non-unified slots; also test above training context where llama.cpp permits it | `stories15M-q4_0`; Qwen3.5; Gemma 4 | Ordinary KV grows with context; modeled recurrent state does not. Current llama.cpp first pads total context, pads each non-unified per-slot slice, and replaces total context by slice × slots; causal batch remains capped by the original unpadded request. |
| Fit-to-free-memory | omit `--ctx-size`/`--n-gpu-layers` with default `--fit on`; then make both explicit and use `--fit off`; vary `--fit-target`/`--fit-ctx` only in the first case | `stories15M-q4_0`, then one model too large for the selected device | Fit can reduce an unset model-default context, GPU layer count, split, and MoE placement according to free memory at that instant. The implicit case must be `low`; the explicit fit-off case is reproducible. |
...
| Recurrent speculative rollback | Qwen3.5 + DSpark at `--parallel 1`, compare `--spec-draft-n-max 1`, `3`, `7` | Qwen3.5 0.8B + compact DSpark | Target RS is multiplied by `1 + draft depth` only on current rollback-capable `qwen35`/`qwen35moe`/`deepseek4`; the reviewed target logged 38.53/77.06/154.12 MiB. Other recurrent families clamp rollback snapshots to zero. |
| Parallel slots | omit `--parallel`, set `-1`/another negative, then compare positive `1`/`4` with `--kv-unified` on and off; combine negative auto with `--no-kv-unified` | Qwen3.5 and Gemma 4 | Omitted and all explicit negative values resolve to four unified slots. Auto is applied after parsing and overrides the negative KV flag. Recurrent state and SWA stream state scale with sequence count; unified global KV does not. |
...
| Compute | `--ubatch-size 128`, `512`, `1024`, and `0`; vary `--batch-size` separately | Any model; Gemma 4 stresses the large vocabulary | Dominant compute reservation is linear in `n_vocab * n_ubatch`, not context or parallel slots. Explicit ubatch `0` resolves to the logical batch; at batch/ubatch 1024 the reviewed tiny CUDA model used 32.52 + 6.27 MiB compute versus 8.69 + 1.57 MiB at ubatch 256. Changing logical batch 512/2048/8192 at fixed ubatch 512 did not change fit buffers. |
| Server output mode | normal completion vs `--embedding` with `--pooling mean`/`none`, reranking, and `--backend-sampling`; vary batch/ubatch and slots, including ubatch `0` | stories15M, e5-small-v2, Jina tiny reranker | Encoder/reranker paths remain cacheless and force `n_batch=n_ubatch` before context creation. In b10276 that server clamp precedes the library's ubatch-zero fallback, so embedding/rerank + ubatch `0` produces zero batch and is rejected by Arriero. Main backend sampling adds `(3*n_vocab+1)*parallel*4` startup host bytes; output growth is request-time. |
...
| CPU, partial, full GPU | `--n-gpu-layers 0`, a finite middle value, and `all`/`999` | `stories15M-q4_0` first, then Qwen3.5 or TinyGemma | Tensor weights move by layer and input embedding remains on host. If `output.weight` is absent, the tied token embedding also appears on the output GPU. Omitted/`auto` GPU layers use a conservative full-offload estimate and default fit makes placement `low`. |
| Operation offload | with a visible GPU and `--n-gpu-layers 0`, compare default `--op-offload` and `--no-op-offload`; repeat at full weight offload | `stories15M-q4_0` | CPU weights do not imply zero VRAM: default op offload puts primary compute on the first GPU. On b10276 the tiny model moved from 72 MiB CUDA compute + 6 MiB host compute to zero CUDA + 63 MiB host with `--no-op-offload`; full GPU offload was unchanged. |
| Host buffer representation | default vs `--no-repack`, `--no-host`, and supported `--load-mode` values | `stories15M-q4_0`, then Q4_0/K-quant models with known CPU repack inflation | Repack/no-host change backend buffer types and can change model/compute columns, so explicit variants remain `low`. Load mode changes RSS residency/page-cache/locking rather than logical tensor bytes; measure RSS separately. |
...
| Multiple GPUs | two or more GPU pools; first omit `--tensor-split`, then compare explicit equal/unequal layer splits; separately use `--split-mode none --main-gpu N`, row, and tensor | Any dense anchor; repeat with MoE | `none` places all offloaded model buffers on exactly the selected GPU. Omitted layer split uses startup free-memory proportions. Explicit proportions stabilize weights/KV, but per-device scheduler/pipeline compute is still backend-dependent; every multi-GPU layer/row/tensor case remains `low` until hardware-qualified. |
| Flash attention | `--flash-attn auto`, `off`, and `on`, including quantized KV | TinyGemma/Qwen3.5 for ordinary attention; DeepSeek V2 Lite for MLA | Auto probes the actual backend graph and is `low` unless another mode forces it. On the reviewed DeepSeek case, auto/off/on compute split was respectively CUDA+host 188.27+16.01 / 180.27+24.01 / 76.13+58.01 MiB. GPU MLA adds context-sized host staging for Flash Attention or quantized K cache. |
| Projector | `--mmproj`, then add `--no-mmproj-offload`; vary image token bounds and `--mtmd-batch-max-tokens`; send one real request | TinyGemma/Gemma 4 for image; Qwen3-ASR for audio | Projector weights move GPU -> host. Image/audio/video preprocessing and graph memory are request-time and depend on media batching, so capture pre/post telemetry separately. The removed server vocoder/TTS flags are not another projector case. |
| Draft model | target only vs `--spec-draft-model`; vary `--spec-draft-ngl`, draft cache/MoE args, global op-offload/split/main-GPU, and fit | stories15M MoE + stories15M draft | Draft weights, compute and its own KV are added recursively. Global placement controls are inherited; automatic draft GPU layers with fit enabled depend on free memory and remain `low`. |
| Draft backend sampling | MTP and Eagle3 with default draft backend sampling, then `--no-spec-draft-backend-sampling`; vary slots | Any qualified MTP pair; Eagle3 remains a gap canary | The draft context adds/removes `(3*n_vocab_draft+1)*parallel*4` host bytes. DFlash/DSpark and draft-simple do not install these backend sampler chains in current llama.cpp. |
| N-gram speculative modes | each of `ngram-simple`, `ngram-map-k`, `ngram-map-k4v`, `ngram-mod`, `ngram-cache`, and `--spec-default`; combine both map modes and vary slots | Any target; stories15M is sufficient | No draft GGUF is loaded. `ngram-mod`/preset adds 16 MiB shared host RAM; each map mode adds 1 MiB per slot; simple adds no fixed table. History vectors grow dynamically and `ngram-cache` remains `low` because file/history maps are unbounded statically. |
...
| Family-specific MTP | `--spec-type draft-mtp`; vary draft depth, cache types, SWA mode, and slots | Step 3.5, MiMo2, or Hy3 oversized-candidate row | Current family graphs differ from ordinary dense NextN: Step 3.5 has three heads, MiMo2 also uses fused QKV/iSWA, and Hy3 has its own graph. Known weights/KV are included, but the result must remain `low` until one complete target/MTP combination reaches a real request on matching hardware. |
...
| Explicit parallel default | omit `--parallel`; then set `--parallel -1` and `4`; finally add `--kv-unified` | Gemma 4 or GPT-OSS | Omitted and negative auto parallel resolve to four unified slots. An explicit positive slot count is non-unified unless `--kv-unified` is also passed; Arriero must follow that server rule. |
| Metadata and non-layer placement | `--override-kv`; `--split-mode row`/`tensor`; `--override-tensor`; draft device/override options | Any GPU anchor; Qwen3.5 for a memory-defining SSM override | Metadata overrides can invalidate geometry read from GGUF and backend overrides invalidate layer-based per-pool placement. Until modeled, the estimate must be `low` rather than silently using the original metadata/layer split. `split-mode none` is covered separately and is not part of this gap. |
...
| Dynamic server caches and sleep | qualify statically with `--cache-ram 0 --ctx-checkpoints 0`; separately exercise default prompt RAM cache, idle-slot snapshots, checkpoints, and `--sleep-idle-seconds` | Any target; add a draft/mmproj to test composite teardown | Limits are lazy, not startup reservations. Static buffer comparison excludes them; peak RSS validation must be request-driven. Sleep destroys model/context/draft/mmproj, so Arriero reports the awake footprint as `low` rather than comparing sleeping RSS with it. |
| Environment argument parity | put model/context/batch/cache/device/fit values in `LLAMA_ARG_*`, repeat with CLI overrides, test a `LLAMA_ARG_NO_*` compatibility toggle, and leave a same-option UI value at boolean `false` | Any small target | The resolved estimate must equal the equivalent launch. A CLI value actually emitted by argv wins over env; boolean `false` is omitted by Arriero and must not mask an env value. No extra model is needed. |
| Artifact, parser, and preset resolution | missing local model/mmproj/draft/LoRA/control vector; separate draft type without a sidecar; remote selectors; router/built-in presets; removed and print-and-exit flags; invalid context/batch/slot limits; a string value on a flag-style boolean | No new artifact | Estimation must fail with 422 before producing a plausible number for a process that current llama.cpp will not start. Built-in presets rewrite memory arguments, remote selectors may auto-discover sidecars, `--parallel` accepts negative auto or 1..256, and paired flag booleans must use the selected positive/negative alias rather than a separate `false` argv token. |
| Device validation | `--device none`, reordered CUDA devices, an unavailable CUDA index, an invalid CPU/backend name, and out-of-range `--main-gpu` under `split-mode none` | Any small GPU anchor | `none` is the supported CPU control. Current llama.cpp rejects CPU as an explicit device; Arriero must not silently reinterpret invalid or unmapped accelerators as a host-only estimate. |
...
| Old `--model-vocoder`/`--hf-repo-v`/`--hf-file-v`/`--tts-use-guide-tokens` server rows | No current server artifact | These flags were removed from source and the b10276 binary; stale rows in the generated upstream README are not executable coverage. Current server audio input is an mmproj case, while speech generation belongs to the separate `llama-tts` executable. |
...
| N-gram speculative modes | Target model alone | No mode loads a draft GGUF or second model context. Arriero models the fixed 16 MiB `ngram-mod` table and 1 MiB-per-slot map tables; only histories and `ngram-cache` file maps remain dynamic. One small target covers the argument family. |
| `--fit`, op/Flash offload, split/main-GPU, repack/no-host/load mode, backend sampling, sleep, server cache flags, and their `LLAMA_ARG_*` forms | stories15M plus the existing encoder/reranker/DeepSeek roles | These are placement, backend-buffer, output-buffer, runtime-residency, or parser-precedence axes. They require argument matrices and sometimes RSS/buffer-log capture, not another model. Use a larger quantized model only to amplify backend repack behavior. |
| Sampling chain, penalties, grammar/JSON schema, token limits, context shift, continuous batching, warmup, tensor checking, and NUMA policy | Any existing target or no estimator case | These change graph operations, scheduling, validation, CPU locality, or transient/request state, but not GGUF weights or the analytical persistent KV formula. Re-test when upstream introduces a new persistent sampler/state allocation; otherwise they do not justify model downloads. |
| CPU/HTTP/spec-draft thread counts, affinity, priority, and polling | Any existing target or no estimator case | They affect thread stacks, process RSS, scheduling, and CPU locality, not the model/context/compute buffers printed by the analytical engine. Cover them in measured-RSS tests rather than downloading another GGUF. |
| Embedding pooling modes and rerank launch flags | e5-small-v2 and Jina tiny reranker | `--embedding`, pooling choice, and rerank select already-covered cacheless output/activation paths. They need role-specific server requests, not another architecture, unless a new classifier/pooling tensor layout appears. |
...
the estimation endpoint. Remote mmproj/draft selectors and built-in/router
presets are rejected too. Every explicitly configured local artifact must exist;
missing mmproj/draft/LoRA/control-vector files fail instead of being silently
omitted. The same rules apply when paths or selectors come from `LLAMA_ARG_*`.
...
| --- | --- |
| stories15M dense and MoE | Loaded; ordinary F16 KV matched exactly. The CUDA server completed a real draft-simple request with accepted draft tokens. Quantized KV correctly failed in llama.cpp because head width 48 is incompatible with the cache block size. On b10276, CPU weights with default op offload used 72 MiB CUDA + 6 MiB host compute, while `--no-op-offload` used zero CUDA + 63 MiB host compute. CPU-only default/no-repack model buffers were 14/17 MiB; CUDA default/`--no-host` host model+compute were 17+6/14+9 MiB. Backend sampling grew the startup host output buffer from 0.12 to 0.49 MiB at one slot, matching its analytical increment. |
...
| Qwen3.5 0.8B | Loaded; modeled recurrent + attention context matched across context sizes, KV types, and ubatch cases. One-GPU weights split analytically as 497 MiB GPU + 199 MiB host versus fit 497 + 198 MiB; compute stayed within +0.4–0.8%. With compact DSpark and one slot, target RS scaled exactly from 38.53 to 77.06 to 154.12 MiB at `--spec-draft-n-max` 1/3/7. |
...
| DeepSeek V2 Lite | Loaded; legacy MLA context matched exactly at displayed MiB for every F16/Q8_0/Q4_0 case. GPU host staging closed the large quantized-KV/Flash-Attention compute omission; aggregate compute is within -3.6% to +5.1%. At ctx/batch/ubatch 4096/512/512, auto probed Flash Attention off and reported CUDA+host compute 188.27+16.01 MiB; forced on/off reported 76.13+58.01 / 180.27+24.01 MiB. |
...
one-GPU tied-output rule, CPU-weight op-offload placement, recurrent rollback,
backend-sampling output, and MLA host staging. Current argument-environment
precedence, negative auto-parallel, embedding/rerank batch clamping,
single-GPU `split-mode=none`, invalid-device rejection, Flash auto disclosure,
sleep disclosure, and draft placement inheritance are covered by unit/API tests.
Fixed n-gram sizes were checked against the b10276 source and exact unit
formulas. Multi-GPU scheduler compute, row/tensor splits,
