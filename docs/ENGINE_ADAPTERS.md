...
| `proxy` | Capability booleans + translation dialect consumed by the proxy (below) | native capabilities, `llama-server` dialect | all `false`, `llama-server` dialect | serve + lease + estimated rate, `openai-compatible` dialect | serve + lease + estimated rate, `openai-compatible` dialect | serve + lease, `openai-compatible` dialect |
...
- `estimateStreamRate` — enables the shared stream inspector’s observed generation interval when upstream timings are absent. Enabled for SGLang and vLLM; trace rates are marked as estimates. Measurement boundaries and exclusions: `docs/API_PROXY_FOUNDATION.md` § Telemetry.
