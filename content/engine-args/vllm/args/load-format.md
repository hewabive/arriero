...
- `--quantization`: чтение конфигурации квантизации происходит независимо от формата; для `torchao`-чекпоинтов формат остается safetensors, а меняется стратегия чтения.
