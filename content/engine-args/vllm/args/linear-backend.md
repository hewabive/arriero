...
Это ручка сравнительных замеров и обхода багов в конкретном ядре, а не «ускоритель». Большинство значений жёстко привязано к железу (`aiter` — только ROCm, `xpu`/`xpu_woq` — только XPU, `flashinfer_b12x` — SM120+), а `emulation` в справке прямо помечен как «for testing only».
