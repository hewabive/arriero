...
- Имена из реестра чекпойнтов (`compressed-tensors`, `gptq`, `awq`, `bitsandbytes`, `modelopt`, `quark`, `torchao`, `inc`, …) описывают формат уже квантованных весов. `bitsandbytes` стоит особняком: он принимает и неквантованные веса (квантует их в nf4 при загрузке) и жестко привязан к собственному загрузчику — явное `--quantization bitsandbytes`, как и bnb-`quantization_config` чекпойнта, принудительно переключает `--load-format` на `bitsandbytes` (`BitsAndBytesModelLoader`).
