...
- **VRAM (KV-cache).** На новом раннере не выделяется вовсе. Освободившийся акселератор — единственный контекст, в котором vLLM разрешает гонять мультимодальный препроцессинг на устройстве (см. `--mm-processor-device`).
...
- `--mm-processor-device`, `--mm-tensor-ipc`: разрешают перенести препроцессинг на свободный акселератор энкодерного узла (нужны оба).
