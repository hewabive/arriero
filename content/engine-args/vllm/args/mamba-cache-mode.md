...
summary: Стратегия сохранения состояния mamba-слоев для prefix caching. Декларативный дефолт none почти всегда переопределяется движком: при включенном prefix caching режим становится align.
group: CacheConfig
related:
...
- Эффективное значение: `MambaModelConfig.verify_and_update_config` меняет его почти всегда. При включенном prefix caching: `none` → `align`; `all` на модели без `supports_mamba_prefix_caching` → `align` с предупреждением. При выключенном prefix caching любой режим принудительно становится `none`, тоже с предупреждением
...
- prefix caching включен, режим `none` → `align`, лог `Mamba cache mode is set to 'align' for <Architecture> by default when prefix caching is enabled`;
...
- `align` — состояние сохраняется только для последнего токена каждого шага планировщика и только когда он попал на границу блока. Дешево по памяти и по числу записей; дефолт при включенном prefix caching.
- `all` — состояние сохраняется на каждой границе `i × block_size`. Больше точек продолжения (лучше hit rate), дороже по памяти и по записям; требует, чтобы модель объявляла поддержку.

## Когда использовать

- Оставляйте автоматический выбор. Он корректен для подавляющего большинства случаев: `align` при prefix caching и `none` без него.
- `all` имеет смысл на модели с `supports_mamba_prefix_caching`, когда общие префиксы длинные и вы измеряете рост hit rate, готовы платить памятью за таблицу состояний.
- `none` при включенном prefix caching задавать бессмысленно: движок все равно поднимет режим до `align`.
...
- **Проверка итогового режима:** info-строка `Mamba cache mode is set to '<mode>' for <Architecture> by default when prefix caching is enabled` в логе старта.
