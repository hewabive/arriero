...
- `vllm/entrypoints/launchers/cli_args.py` — `FrontendArgs` и `make_arg_parser`, то есть все, что относится к HTTP-слою; `vllm/entrypoints/openai/api_server.py` и подпапки `chat_completion/`, `completion/`, `responses/`, `models/`, `parser/` — как значение доходит до конкретного endpoint.
