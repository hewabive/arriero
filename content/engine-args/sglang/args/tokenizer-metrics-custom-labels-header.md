...
Ключевой факт: форвардер прокси arriero передает заголовки клиента на upstream почти нетронутыми — вырезаются только hop-by-hop, `host`, `content-length` и два служебных заголовка возобновляемого потока (`proxyRequestHeaders` в `apps/api/src/proxy/http.ts`). Заголовка с пользовательскими метками среди них нет. Значит, при включенной связке любой клиент прокси может проставить метки на управляемом инстансе SGLang напрямую, и «доверенного звена» между ним и движком не появляется.
...
Второе следствие — эксплуатационное: строка `Error in request: …` с трассировкой попадает в лог инстанса, а разбор лога arriero (`apps/api/src/process/log-parsers/sglang.ts`) реагирует и на слово `error`, и на `traceback`. Один клиент с битым заголовком переведет здоровый инстанс в состояние `degraded` (`apps/api/src/process/health-summary.ts`).
