...
- **`layered`** — `LayeredModelLoader`: модель создается на meta-device и заполняется помодульно, что уменьшает пиковую память загрузки, когда слой можно обработать (например, квантовать) до чтения следующего. Требует у класса модели метод `load_weights_to_module`, иначе `ValueError: LayeredModelLoader requires the model to have a load_weights_to_module method.`
