...
Если вы все же включаете пользовательские метки на управляемом инстансе, помните: форвардер прокси передает заголовки клиента на upstream почти без изменений — вырезаются только hop-by-hop, `host`, `content-length` и два служебных заголовка возобновляемого потока (`apps/api/src/proxy/http.ts`). То есть заголовок с метками пройдет от клиента до движка насквозь, и разделения доверия между клиентом прокси и движком не будет.
