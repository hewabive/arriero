...
- Нужны метрики сбалансированности в проде: не трогайте этот аргумент, задайте `--expert-balancedness-report-mode prometheus` или `both` — режим сам поставит `stat`.
...
- `--expert-balancedness-report-mode`: любое значение кроме `off` подставляет `stat` и автоматически стартует запись при инициализации рекордера, без HTTP-вызова.
