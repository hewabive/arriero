...
Проверяется поле `ServerArgs`, то есть **явно заданный** `--mamba-ssm-dtype bfloat16`. Тот же bf16, унаследованный из конфига модели, автоподстановку не включает. База `helion` отдельно исключена, чтобы KDA Helion не был перезаписан FlashInfer. В логе: `SM100+ detected with mamba-ssm-dtype=bfloat16, defaulting --linear-attn-decode-backend to flashinfer.`
...
- **KDA**: `triton`, `helion`, `cutedsl`, `flashinfer`. Helion требует CUDA и `helion==1.4.0`; прочее — `ValueError: Unsupported KDA decode backend: …`.
...
- `helion` создает KDA Helion packed-decode/ReplaySSM kernel; на GDN и не-CUDA отвергается.
...
- Оставлять `triton` для ReplaySSM на GDN; на KDA допустим также `helion`.
- На KDA можно сочетать `--enable-linear-replayssm` с `helion`, но справка upstream предупреждает, что KDA ReplaySSM обычно медленнее packed decode — сначала измерьте.
- Задавать `cutedsl` или `helion` на KDA-гибридах (Kimi Linear, Kimi K3) при page-major-раскладке можно только для MLA-гибридов; обе опции разрешены соответствующим ассертом decode.
...
- `--enable-linear-replayssm`: требует `triton`, либо `helion` для KDA.
...
- `ValueError: --enable-linear-replayssm requires Triton, or Helion for KDA, as the linear-attn decode backend ...`
...
- `The Helion package is required ... pip install helion==1.4.0` — выбран `helion`, но dependency отсутствует.
