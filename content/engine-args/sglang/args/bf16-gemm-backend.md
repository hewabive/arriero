...
- `cutedsl` включает оптимизированный CuTe DSL путь; вне SM100/SM103 или при deterministic inference старт завершается ValueError. Прежний алиас `flashinfer_pr4266` больше не принимает argparse.
...
`auto` сохраняет автоподбор, `torch` принудительно использует cuBLAS. `cutedsl` — оптимизированный путь Blackwell; `gemv` — явный Hopper-вариант. Удалённый алиас `flashinfer_pr4266` отвергает argparse.
