...
summary: Выбирает ядро для неквантованных BF16-линейных слоев. Значимо только на SM10x: там `auto` включает JIT CuTe DSL TGV GEMM, который сам решает по форме матрицы, брать ли себя или cuBLAS. На всех остальных картах это всегда cuBLAS.
group: exec.kernel
related:
  - --dtype
  - --quantization
...
- Эффективное значение: `initialize_bf16_gemm_config` (`sglang/python/sglang/srt/layers/quantization/unquant.py`) превращает `auto` в `cutedsl` при `is_sm100_supported()`, иначе оставляет `auto` — и `auto` в этом случае ведет себя как `torch`, потому что `Bf16GemmBackend.is_cutedsl()` ложно и весь диспетчер сводится к `F.linear`
...
- **`cutedsl`.** Требует SM10x: `initialize_bf16_gemm_config` бросает `ValueError: --bf16-gemm-backend cutedsl requires an SM10x GPU` на любой другой карте — это отказ на старте планировщика, не тихий fallback. Дальше подгружаются `cutedsl_bf16_gemm` и предикат `use_cutedsl_bf16_gemm`.
...
- `auto` — рекомендуемое. На SM10x это `cutedsl`, на остальном — cuBLAS.
- `cutedsl` вне SM10x — `ValueError` на старте. Никакой деградации до cuBLAS не будет.
