...
- `disabled` — когда старт нужно ускорить любой ценой (диагностика, короткие проверки конфигурации) или когда захват падает по памяти и уменьшать `--cuda-graph-max-bs-decode` некуда.
