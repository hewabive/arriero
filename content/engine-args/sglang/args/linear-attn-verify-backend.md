...
Общий список CLI теперь включает `intel_xpu`, но это само по себе не добавляет KDA verify-ядро: его диспетчер по-прежнему принимает только `triton`, `nv_cutedsl` и `flashinfer`. Для GDN XPU неперехваченные специализированным backend вызовы используют Triton.
