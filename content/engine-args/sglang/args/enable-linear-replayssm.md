...
summary: Буферизованный decode-путь линейного внимания: вместо записи полного состояния в HBM каждый шаг ядро копит записи в кольце длины L и сбрасывает состояние раз в L шагов. Дает 1.2–1.5× на GDN при батче от 64; KDA использует выбранную Triton- или Helion-реализацию, но обычно проигрывает packed-decode.
group: exec.mamba
related:
...
1. decode-backend линейного внимания должен быть `triton`, либо `helion` для KDA;
...
- RAM хоста: не влияет.
- Время старта: дополнительные аллокации и компиляция выбранного Triton- либо Helion-ядра кольцевого варианта при первом decode.
...
- `--linear-attn-decode-backend`: обязателен `triton`, либо `helion` на KDA. Helion требует CUDA и пакет `helion==1.4.0`; на GDN он отвергается.
...
- `ValueError: --enable-linear-replayssm requires Triton, or Helion for KDA, as the linear-attn decode backend ...` — выбран другой backend.
