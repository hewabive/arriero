...
- Эффективное значение: `initialize_fp8_gemm_config` (`sglang/python/sglang/srt/layers/quantization/fp8_utils.py`) переписывает `auto` на `cutlass` при SM120, а также на `flashinfer_cutlass`, если `--quantization mxfp8` на SM100 с доступным FlashInfer. В остальных случаях `auto` остается `auto` и разрешается позже, в `_dispatch_auto_backend`, в момент выбора функции
...
Отдельная ветка — **MXFP8-плотные слои**: `resolve_mxfp8_dense_gemm_backend` учитывает этот флаг только когда он называет backend, у которого есть MXFP8-ядро (`flashinfer_trtllm`, `deep_gemm`); при прочих значениях он выбирает сам (gfx95 на ROCm, FlashInfer CUTLASS на Blackwell, DeepGEMM на Hopper) и, если ничего не подходит, ставит `_unsupported_mxfp8_linear`, который бросает исключение при первом вызове.
