...
title: "--load-mode"
summary: "Выбирает единый режим загрузки модели: авто-режим, обычное чтение, mmap, mlock без mmap, mmap с mlock или Direct I/O. Это замена удалённых `--mmap`, `--mlock` и `--direct-io`."
...
Этот аргумент заменяет три удалённых legacy-флага загрузки. Режимы теперь взаимоисключающие, поэтому конфигурация не может одновременно просить mmap, mlock и Direct I/O.
...
Устаревшие `--mlock`, `--mmap`/`--no-mmap` и `--direct-io`/`--no-direct-io` удалены из текущего `common/arg.cpp`. В существующих конфигурациях замените их на `--load-mode mlock`, `mmap`, `none` или `dio` соответственно. Для прежней комбинации mmap с блокировкой страниц используйте `--load-mode mmap+mlock`.
...
Это тихое изменение поведения для уже существующих конфигураций: инстанс или INI-пресет с `load-mode = mlock` (в старой сборке также с legacy `--mlock`) после обновления бинарника получит полное чтение файла вместо mmap — заметно более медленный старт и анонимную копию модели в RSS. Если нужно прежнее поведение, поменяйте значение на `mmap+mlock` явно.
