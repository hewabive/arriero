...
Это означает, что implicit split не обязательно равный и может измениться между запусками на одном хосте. Arriero помечает multi-GPU оценку без явного `--tensor-split` как `low`; для воспроизводимой per-pool оценки задайте ненулевые доли.
