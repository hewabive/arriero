...
Ожидается путь к файлу GGUF. Относительные пути разрешаются относительно текущего рабочего каталога процесса `llama-server`, поэтому в arriero надежнее хранить абсолютные пути.
...
Если выбран `--load-mode mmap`, чтение локального файла может быть ленивым и зависеть от page cache. Для стабильной диагностики сравнивайте одинаковый файл, одинаковый backend и одинаковые параметры offload.
